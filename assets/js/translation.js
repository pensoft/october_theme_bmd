...
class TranslationManager {
    ...
    createGoogleTranslateElements() {
        ...
        // Create main Google Translate element (desktop)
    ...
    initLanguageSwitcher() {
        ...
        if ($(SELECTORS.GOOGLE_TRANSLATE_ELEMENT).length && 
            $(SELECTORS.GOOGLE_TRANSLATE_ELEMENT + ' ' + SELECTORS.LANGUAGE_SWITCHER).length === 0) {
     ...
     */
    createLanguageSwitcher(containerSelector = SELECTORS.GOOGLE_TRANSLATE_ELEMENT) {
        const currentLanguage = this.getCurrentLanguageInfo();
        const dropdown = this.createDropdownHTML(currentLanguage);
        
        $(containerSelector).html(dropdown);
        this.bindDropdownEvents(containerSelector);
    }
     ...
     */
    bindDropdownEvents(containerSelector = SELECTORS.GOOGLE_TRANSLATE_ELEMENT) {
        const $dropdown = $(containerSelector).find(SELECTORS.LANGUAGE_SWITCHER);
    ...
    updateDropdownDisplay($dropdown, languageInfo) {
        // Update the specific dropdown that was clicked
        $dropdown.find('.language-text').text(languageInfo.code.toUpperCase());
        
        // Synchronize both desktop and mobile dropdowns
        this.synchronizeLanguageDropdowns(languageInfo);
    }
